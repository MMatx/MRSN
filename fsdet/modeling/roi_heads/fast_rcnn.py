...
import logging
from detectron2.layers import batched_nms, cat,ShapeSpec,Linear, nonzero_tuple
# from owod.layers import Linear, ShapeSpec, batched_nms, cat, nonzero_tuple
...
def fast_rcnn_inference_single_image(
    ...
    # print('fast_rcnn 138 fast_rcnn_inference_single_image ')
    return result, filter_inds[:, 0]

def fast_rcnn_inference_single_image_n(
        boxes, scores, image_shape, score_thresh, nms_thresh, topk_per_image
):
    """
    Single-image inference. Return bounding-box detection results by thresholding
...
class FastRCNNOutputs(object):
    ...
    def softmax_cross_entropy_loss(self):
        ...
        """
        self._log_accuracy()
        # print('faster_rcnn ',self.pred_class_logits.size(),self.gt_classes)
        tmp=F.cross_entropy(
            self.pred_class_logits, self.gt_classes, reduction="mean"
        )
        # tmp=tmp

        # print('faster 229 class ',tmp)
    ...
    def smooth_l1_loss(self):
        ...
        # print('fast_rcnn 289 loss_box_reg ',loss_box_reg / self.gt_classes.numel())
        return loss_box_reg
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class FastRCNNOutputLayers_con_cluster(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
    ...
    def __init__(
        ...
        *,
        ...
        # logging.getLogger(__name__).info("Invalid class range: {} ".format(str(self.invalid_class_range)))

        self.max_iterations = max_iterations
        self.feature_store_is_stored = False
        self.output_dir = output_dir
        self.feat_store_path = feat_store_path
        tmp_save_root=os.path.join(self.output_dir, self.feat_store_path)
        if not os.path.exists(tmp_save_root):
            os.mkdir(tmp_save_root)
        ...
        if os.path.isfile(self.feature_store_save_loc):
            logging.getLogger(__name__).info('Trying to load feature store from {} '.format(self.feature_store_save_loc))
            self.feature_store = torch.load(self.feature_store_save_loc)
        else:
            logging.getLogger(__name__).info('Feature store not found in  {}. Creating new feature store.'.format(self.feature_store_save_loc ))
            self.feature_store = Store(num_classes + 1, clustering_items_per_class)
        self.means = [None for _ in range(num_classes + 1)]
        self.margin = margin

        print('faster  FastRCNNOutputLayers_con_cluster  507')

        # self.ae_model = AE(input_size, clustering_z_dimension)
        # self.ae_model.apply(Xavier)

    @classmethod
    def from_config(cls, cfg, input_shape, num_classes, cls_agnostic_bbox_reg, box_dim=4):
        return {
    ...
    def update_feature_store(self, features, proposals):
        ...
        if storage.iter == self.max_iterations-1 and self.feature_store_is_stored is False and comm.is_main_process():
            logging.getLogger(__name__).info('Saving image store at iteration {}  to  {}'.format(str(storage.iter) ,self.feature_store_save_loc))
    ...
    def clstr_loss_l2_cdist(self, input_features, proposals):
        ...
        """
        gt_classes = torch.cat([p.gt_classes for p in proposals])
        # print('gt_classes  faster_rcnn 587  ',gt_classes)
        ...
        # print('fast_rcnn  737  fg_ize{}  all_means.len()={}'.format(fg_features.size(),len(all_means)))
        # if(len(all_means)>0):
        #     print(all_means[0].size())torch.Size([1024])
        # fast_rcnn  737  fg_izetorch.Size([145, 1024])  all_means.len()=16
    ...
    def losses(self, predictions, proposals, input_features=None):
        ...
        """
        # print('fastercnn 804 self.num_classes  ',self.num_classes)
        ...
        if input_features is not None:
            # print('faster_rcnn input_features',input_features)
            ...
            # print('faster  rcnn 687 ',losses["loss_clustering"])
