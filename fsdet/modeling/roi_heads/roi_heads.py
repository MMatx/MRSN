...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        print('RES50 output_layer  ',output_layer,cfg.OWOD.ENABLE_CLUSTERING)
        ...
        # self.feature_save_path = os.path.join(cfg.OUTPUT_DIR, cfg.OWOD.FEATURE_SAVE_PATH)
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.training:
            ...
            if self.enable_clustering:
                self.box_predictor.update_feature_store(feature_pooled, proposals)
            del features
...
class StandardROIHeads(ROIHeads):
    ...
    def _init_box_head(self, cfg):
        ...
        output_layer = cfg.MODEL.ROI_HEADS.OUTPUT_LAYER
        print('roi 429 cfg.MODEL.ROI_HEADS.OUTPUT_LAYER  ',cfg.MODEL.ROI_HEADS.OUTPUT_LAYER)
        self.box_predictor = ROI_HEADS_OUTPUT_REGISTRY.get(output_layer)(
            cfg,
        ...
        self.enable_clustering = cfg.OWOD.ENABLE_CLUSTERING
        print('roi cfg.OWOD.ENABLE_CLUSTERING ',cfg.OWOD.ENABLE_CLUSTERING)
...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeads_cluster(ROIHeads):
    """
    It's "standard" in a sense that there is no ROI transform sharing
    or feature sharing between tasks.
    The cropped rois go to separate branches directly.
    ...
    def __init__(self, cfg, input_shape):
        super(StandardROIHeads_cluster, self).__init__(cfg, input_shape)
        self._init_box_head(cfg)
    ...
    def _forward_box(self, features, proposals):
        ...
        Returns:
            ...
            In inference, a list of `Instances`, the predicted instances.
        """
        # print('roi proposal ',len(proposals),type(proposals),type(proposals[0]))
        # roi proposal  4 <class 'list'> < class 'detectron2.structures.instances.Instances' >
        # print(proposals[0])num_instances=512, image_height=807, image_width=800, fields=[proposal_boxes

        box_features = self.box_pooler(
            features, [x.proposal_boxes for x in proposals]
        )
        box_features = self.box_head(box_features)
        # print('box_features  ',box_features.size()) #torch.Size([2048, 1024])
        #boxes,objectness_logits,gt_classes,gt_boxes
